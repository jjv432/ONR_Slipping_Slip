function [UserRigidP] = optitrackInterp(FileName, MarkerInfo, BodyNames, normalizeBool, saveBool)

%% Dev Notes
%{
   Implementation of the optitrack_R2 function to not require user inputs
   via a prompt. All other functionality is the same, though
%}
...
MarkerCount = MarkerInfo(1);
RigidBodyCount = MarkerInfo(2);
...
if (normalizeBool == 'Y')||(normalizeBool == 'y')||(normalizeBool ==1)
    normalizeBool = 1;
    fprintf('\n***Normalizing***\n\n')
else
    fprintf('\n***Not normalizing***\n\n')
...
for z = 1:RigidBodyCount
    ...
    SpecificName = BodyNames(z);
    %Names(z) = string(SpecificName);
    Names(z) = SpecificName;
...
if (saveBool == 'Y')||(saveBool == 'y')||(saveBool ==1)
